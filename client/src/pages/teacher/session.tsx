...
export default function TeacherSession() {
  ...
  const { data: activePoll, refetch: refetchActivePoll } = useGetActivePoll(
    sessionId,
    {
      query: {
        enabled: isReady && !!sessionId,
...
useEffect(() => {
    ...
    answerMutation.mutate(
      {
        ...
        data: {
          answer: teacherAnswer || "Answered live",
          answeredBy: "teacher" as AnswerQuestionRequestAnsweredBy,
        },
      },
      {
        onSuccess: () => {
  ...
  const launchPoll = () => {
    ...
    if (!pollQuestion.trim() || cleanOpts.length < 2) {
      toast({
        variant: "destructive",
        title: "Fill in the question and at least 2 options",
      });
      return;
    }
    ...
    createPollMutation.mutate(
      {
        sessionId,
        data: { question: pollQuestion.trim(), options: cleanOpts },
      },
      {
        onSuccess: (poll) => {
          // Immediately activate the poll after creating it
          activatePollMutation.mutate(
            { sessionId, pollId: poll.id },
            {
              onSuccess: () => {
                queryClient.invalidateQueries({
  ...
  const handleClosePoll = () => {
    ...
    closePollMutation.mutate(
      { sessionId, pollId: activePoll.pollId },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: getGetPollsQueryKey(sessionId),
    ...
    <div className="min-h-[100dvh] bg-background text-foreground flex flex-col">
            ...
            <AlertDialogContent className="glass-panel border-white/10 text-white">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="text-muted-foreground">
                  This will permanently close the session. Students will be
                  disconnected and no further questions can be asked.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 max-w-[1600px] mx-auto w-full">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="glass p-6 rounded-3xl flex-1 flex flex-col min-h-[500px]">
            <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4">
              <div className="flex items-center gap-3">
                ...
                <Badge
                  variant="secondary"
                  className="bg-primary/20 text-primary hover:bg-primary/20 border-0 ml-2"
                >
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  disabled={
                    pendingQuestions.length === 0 || currentQuestionIndex === 0
                  }
            ...
            {pendingQuestions.length > 0 && currentQuestion ? (
                ...
                <div className="bg-black/30 p-8 rounded-2xl border border-white/5 mb-6 flex-1 flex flex-col justify-center relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-4">
                    {currentQuestion.type === "simple" ? (
                      <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30 gap-1">
                        <BrainCircuit size={12} /> AI Answered
                      </Badge>
                    ) : (
                  ...
                  <div className="text-sm text-muted-foreground mb-4 font-medium flex items-center gap-2">
                    ...
                    {currentQuestion.duplicateCount > 0 && (
                      <Badge
                        variant="outline"
                        className="ml-2 border-white/20 text-xs"
                      >
                        ...
                        answerMutation.mutate(
                          {
                            ...
                            questionId: currentQuestion.id,
                            data: {
                              answer: currentQuestion.answer || "Answered live",
                              answeredBy: "ai",
                            },
                          },
                          {
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50">
                ...
                <p className="text-muted-foreground">
                  When students ask questions, they will appear here.
                </p>
              </div>
            )}
          </div>
          ...
          {questions &&
            questions.filter((q) => q.status !== "merged").length > 0 && (
              <div className="glass p-5 rounded-3xl">
                <h2 className="text-base font-semibold text-white mb-3 flex items-center gap-2">
                  <MessageSquare size={16} className="text-muted-foreground" />{" "}
                  All Questions
                  <span className="text-xs text-muted-foreground font-normal ml-1">
                    ({questions.filter((q) => q.status !== "merged").length})
                  </span>
                </h2>
                <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                  {questions
                    .filter((q) => q.status !== "merged")
                    .map((q) => (
                      <div
                        key={q.id}
                  ...
                  return (
                    <div key={i} className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-white/80 truncate max-w-[75%]">
                          {opt}
                        </span>
              ...
              <div className="space-y-3">
                ...
                {pollOptions.length < 6 && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-muted-foreground hover:text-white text-xs w-full h-8"
                    onClick={addOption}
                  >
                ...
                <div className="flex gap-2 pt-1">
                  <Button
                    variant="outline"
                    size="sm"
                    className="flex-1 border-white/10 hover:bg-white/5 text-muted-foreground text-xs h-9"
                    onClick={() => {
                      setShowPollForm(false);
                      setPollQuestion("");
                      setPollOptions(["", ""]);
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      activatePollMutation.isPending
                    }
                  >
                    {createPollMutation.isPending ||
                    activatePollMutation.isPending ? (
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    ...
                    {closedPolls
                      ...
                      .map((p) => (
                        <div
                          key={p.id}
                          className="bg-black/20 border border-white/5 rounded-xl px-3 py-2"
                        >
                          <p className="text-xs text-white/70 leading-snug truncate">
                            {p.question}
                          </p>
                          <Badge className="mt-1 text-[10px] bg-white/5 text-muted-foreground border-white/10">
                            Closed
                          </Badge>
                        </div>
                      ))}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground truncate max-w-[150px]">
                    {engagement && engagement.raisedHands > 0
                      ? engagement.raisedHandNames.join(", ")
                      : "None"}
                  </div>
                </div>
              </div>
